...
function requireApiKey(req, res, next) {
  ...
  if (!key || key !== API_KEY) {
    console.warn(`   🚫 Unauthorized request to ${req.method} ${req.path} from ${req.ip}`);
...
app.get("/orders", async (req, res) => {
  try {
    ...
    while (page <= totalPages) {
      const url = `https://ssapi.shipstation.com/orders?orderDateStart=${encodeURIComponent(fmt(start))}&orderDateEnd=${encodeURIComponent(fmt(end))}&pageSize=500&page=${page}`;
    ...
    console.log(`📊 /orders — ${allOrders.length} orders returned for ${fmt(start)}`);
    res.json({ date: fmt(start), orderCount: allOrders.length, rows });
